...
class ChatConversationTransformer extends TransformerAbstract{
	...
	public function transform(ChatConversation $message) {
		$user = Auth::user();
	...
	public function includeInfo(ChatConversation $message) {
	...
	public function includeAuthor(ChatConversation $message){
       $user = $message->sender ? : new User;
       if(is_null($user->id)){ $user->id = 0;}
       return $this->item($user, new UserTransformer);
	}

	public function includeParticipant(ChatConversation $message){
       $particpant = $message->participant ? : new ChatParticipant;
       if(is_null($particpant->id)){ $particpant->id = 0;}
       return $this->item($particpant, new ChatParticipantTransformer);
	}
